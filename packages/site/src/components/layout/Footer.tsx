...
import { dictionaries, hrefFor, type Locale } from '@/lib/i18n';
      ...
      <div className="max-w-6xl mx-auto py-8 px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <div>
            ...
            >
              {isEnglish ? 'Source code on' : '本站源码已在'}
            ...
            <div className="my-4"></div>
            <a
                href={isEnglish ? 'https://creativecommons.org/licenses/by-nc-sa/4.0/' : 'https://creativecommons.org/licenses/by-nc-sa/4.0/deed.zh'}
                target="_blank"
                rel="noopener noreferrer"
                className="text-xs flex items-center max-w-fit transition-colors text-muted-foreground hover:text-primary"
            >
            <span>{isEnglish ? 'Articles are licensed under CC BY-NC-SA 4.0' : '文章采用 CC BY-NC-SA 4.0 许可证'}</span>
            </a>
          </div>

          {/* 页面链接 */}
          <div>
            <h3 className="text-lg font-semibold mb-3 text-foreground">{isEnglish ? 'Navigation' : '页面导航'}</h3>
            <ul className="space-y-2">
              <li>
                <Link href={hrefFor(locale, '/')} className="text-sm transition-colors text-muted-foreground hover:text-primary">
                  {dictionary.nav.home}
                </Link>
              </li>
              <li>
                <Link href={hrefFor(locale, '/blog')} className="text-sm transition-colors text-muted-foreground hover:text-primary">
                  {dictionary.nav.blog}
                </Link>
              </li>
              <li>
                <Link href={hrefFor(locale, '/about')} className="text-sm transition-colors text-muted-foreground hover:text-primary">
          ...
          {/* 社交链接 */}
          <div>
            <h3 className="text-lg font-semibold mb-3 text-foreground">{isEnglish ? 'Contact' : '联系方式'}</h3>
            <ul className="space-y-2">
              <li>
                <a
              ...
              <li>
                <a
                  href="https://x.com/elecmonkeyjs"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-sm flex items-center transition-colors text-muted-foreground hover:text-primary"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
